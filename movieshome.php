...
<?php 
    ...
    //create movies list ------------------------------
    $sql_all = "SELECT * FROM movies ORDER BY id DESC";
    $sql = "SELECT * FROM movies ORDER BY id DESC LIMIT {$start_page}, {$rows_per_page}";
    ...
    if (isset($_GET['filter'])) {
        ...
        $sql_all = "SELECT * FROM movies WHERE name LIKE '%$filter%' OR description LIKE '%$filter%'OR language LIKE '%$filter%'OR tag LIKE '%$filter%'";
        $sql = "SELECT * FROM movies WHERE name LIKE '%$filter%' OR description LIKE '%$filter%'OR language LIKE '%$filter%'OR tag LIKE '%$filter%' LIMIT {$start_page}, {$rows_per_page}";
    ...
    if (isset($_GET['searchsubmit'])) {
        ...
        $search_category = mysqli_real_escape_string($conn,$_GET['search_category']);

        $page_detail .= "&&search_text=".$search_text;
        $page_detail .= "&&search_category=".$search_category;

        $sql_all = "SELECT * FROM movies WHERE name LIKE '%$search_text%' OR description LIKE '%$search_text%'OR language LIKE '%$search_text%'OR tag LIKE '%$search_text%'";
        $sql = "SELECT * FROM movies WHERE name LIKE '%$search_text%' OR description LIKE '%$search_text%'OR language LIKE '%$search_text%'OR tag LIKE '%$search_text%' LIMIT {$start_page}, {$rows_per_page}";

        if ($search_category!='all') {
            $sql_all = "SELECT * FROM movies WHERE language = '$search_category' AND (name LIKE '%$search_text%' OR description LIKE '%$search_text%'OR language LIKE '%$search_text%'OR tag LIKE '%$search_text%')";
            $sql = "SELECT * FROM movies WHERE language = '$search_category' AND (name LIKE '%$search_text%' OR description LIKE '%$search_text%'OR language LIKE '%$search_text%'OR tag LIKE '%$search_text%') LIMIT {$start_page}, {$rows_per_page}";
        }
            ...
            if (mysqli_num_rows($result)>0) {
                while ($details = mysqli_fetch_assoc($result)) {
                    $list .= '<div class="col-md-4 col-sm-6 col-xs-12 blog-post-item">
                                <div class="blog-img">
                                    <a  class="hover-images"><img src="'.$details['img'].'" alt="" class="img-reponsive"></a>
    ...
    //pagination-----------------------
        ...
        if ($page_no<=1) {
            ...
            $first = '<li><a href="movieshome.php?page_no=1'.$page_detail.'">First</a></li>';
        }
        ...
        if ($page_no<=1) {
            ...
            $prev = '<li><a href="movieshome.php?page_no='.$prev_no.''.$page_detail.'"><i class="ion-ios-arrow-back"></i></a></li>';
        }
        ...
        if ($page_no>=$last_page_no) {
            ...
            $next = '<li><a href="movieshome.php?page_no='.$next_no.''.$page_detail.'"><i class="ion-ios-arrow-forward"></i></a></li>';
        }
        ...
        if ($page_no>=$last_page_no) {
            ...
            $last = '<li class="active"><a href="movieshome.php?page_no='.$last_page_no.''.$page_detail.'">Last</a></li>';
        }
...
<body>
    ...
    <div class="pushmenu menu-home5">
        <div class="menu-push">
            ...
            <form role="search" method="get" id="searchform" class="searchform" action="movieshome.php">
                <div>
                ...
                <div class="container container-240">
                    <div class="row flex">
                        ...
                        <div class="col-lg-7 col-md-7 v-center header-search order-1 order-lg-1 hidden-xs hidden-sm">
                            <form method="get" class="searchform ajax-search" action="movieshome.php" role="search">
                                ...
                                <div class="search-panel">
                                            ...
                                            if ($result) {
                                                if (mysqli_num_rows($result)>0) {
                                                    while ($details = mysqli_fetch_assoc($result)) {
                                                        ...
                                                        echo '<option value="'.$details['language'].'">'.$details['language'].'</option>';
                                                    }
                                                }
                                            }
                                        ?>
                                    ...
                                    <!-- <ul id="category" class="dropdown-menu dropdown-category">
                                        <?php 
                                            ...
                                            //     if (mysqli_num_rows($result)>0) {
                                            //         while ($details = mysqli_fetch_assoc($result)) {
                                            //             echo '<li><a href="#">'.$details['language'].'</a></li>';
                                            //         }
                                            //     }
                                            // }
                                        ?>
                                ...
                                <nav class="main-menu">
                                    <div class="collapse navbar-collapse" id="myNavbar">
                                        <ul class="nav navbar-nav">
                                            ...
                                            <li class="level1"><a href="movieshome.php">Latest Movies<span class="h-ribbon h-pos e-green v4">new</span></a></li>
                                            <?php 
                                                $sql = "SELECT distinct(language) FROM movies ORDER BY id DESC LIMIT 10";
                                                $result = mysqli_query($conn,$sql);
                                                if ($result) {
                                                    if (mysqli_num_rows($result)>0) {
                                                        while ($details = mysqli_fetch_assoc($result)) {
                                                            echo '<li class="level1"><a href="movieshome.php?filter='.$details['language'].'">'.$details['language'].'</a></li>';
                                                        }
                                                    }
                                                }
                                            ?>
        ...
        <!-- /header -->
            ...
            <div class="container container-240">
                <div class="blog-banner pd-banner v2">
                   <img src="img/banner/moviebaner.jpg" alt="" class="img-reponsive">
                </div>
                <div class="blog spc1">
                    <ul class="breadcrumb">
                        <li>Home</li>
                    ...
                    <div class="blog-grid">
                        <h1 class="blog-heading text-center">Movies</h1>
                        <div class="row">
